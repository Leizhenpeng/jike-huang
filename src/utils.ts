...
> {
  ...
  {
    const res = await fetch(`${JIKE.Origin}/notes`)
  ...
  if (noteId) {
    const res = await fetch(`${JIKE.Origin}/notes/edit/${noteId}`)
    const htmlText = await res.text()
...
export async function setV2P_Settings(
  ...
  if (updating) {
    ...
    await fetch(`${JIKE.Origin}/notes/edit/${noteId}`, {
      method: 'POST',
      body: formData,
      signal,
    })
  } else {
    // 如果是第一次备份，则新建一个 JIKE 记事本来存储。
    formData.append('parent_id', '0')

    await fetch(`${JIKE.Origin}/notes/new`, {
      method: 'POST',
      body: formData,
      signal,
    })
      ...
      chrome.storage.sync
        ...
        .then((items: StorageItems) => {
          ...
          if (options) {
            data = {
              ...items,
              [StorageKey.Options]: deepMerge(defaultOptions, options),
            }
          } else {
...
export function getCode(fn: Function): string {
  return fn
    .toString()
    .replace(/^[^{]*{\s*/, '')
    .replace(/\s*}[^}]*$/, '')
 ...
 * @param str
 * @example
 * toCamelCase('ORIGINAL_POST') // originalPost
 * toCamelCase('REPOST') // repost
 */
export function toCamelCase(str: string): string {
  return (
    str
      ...
      .replace(/(?:^|_)(\w)/g, (_, c, offset) => (offset > 0 ? c.toUpperCase() : c))
  )
}
